...
public class C206_CaseStudyEvents {
	...
	public static void main(String[] args) {
		...
		while (option != 5) {
			...
			C206_CaseStudyEvents.menu();
			option = Helper.readInt("Enter an option > ");

			if (option == 1) {
				// View all events
				C206_CaseStudyEvents.viewAllevents(eventsList);
            }else if (option == 2) {
            	C206_CaseStudyEvents.setHeader("ADD NEW EVENTS");
					Events E = inputEvents();
					C206_CaseStudyEvents.addEvents(eventsList, E);
					System.out.println("Event added");

				} else if (option == 3) {
					// Delete an event
					C206_CaseStudyEvents.setHeader("DELETE EVENT");
					String eventName = Helper.readString("Enter the name of the event to delete > ");
					C206_CaseStudyEvents.deleteEvent(eventsList, eventName);
				}   else if (option == 4) {
		            // Update an event
					C206_CaseStudyEvents.setHeader("UPDATE EVENT");
		            String eventNameToUpdate = Helper.readString("Enter the name of the event to update > ");
		            C206_CaseStudyEvents.updateEvent(eventsList, eventNameToUpdate);
		        } else if (option == 5) {
	...
	public static void deleteEvent(ArrayList<Events> eventsList, String eName) {
	    ...
	    public static void updateEvent(ArrayList<Events> eventsList, String eName) {
	        ...
	        if (eventToUpdate != null) {
	        	C206_CaseStudy.setHeader("UPDATE EVENTS");
